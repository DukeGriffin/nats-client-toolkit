...
### Async delivery model — RESOLVED into a pluggable `IDelivery` strategy #decision
Instead of hard-picking one hand-off, the read-loop router delivers via a **pluggable delivery strategy** (an `IDelivery` interface with `Deliver(message)`) — callers choose per subscription. **Queue is the default and the RT-safe choice**; **Notifier** (latest-value), **User Event** (UI), and **Actor message** ([[10 Object Messaging]]) are opt-in strategies. Dynamic-dispatch cost is negligible vs. per-message parse/transport; bound every Queue with an overflow policy (slow-consumer protection). Reused by push consumers ([[05 JetStream Consuming]]), KV watch ([[06 Key-Value Store]]), Object watch ([[07 Object Store]]), and async publish acks ([[04 JetStream Publishing]]). Full write-up in [[05 JetStream Consuming]] and [[Library and Project Structure]]. **Remaining:** pin the concrete strategy set + default before building KV/Object watch.
